...
class NeuromorphicMemory:
    ...
    def recall(
        ...
        """
        Recall memories relevant to query using activation spreading.
    ...
    def _activate_and_spread(
        self,
        initial_nodes: List[Tuple[str, float]],
            ...
            for other_id in activated_nodes[i+1:]:
                if other_id in node.connections:
                    node.connections[other_id] = min(
                        1.0,
                        node.connections[other_id] + CONNECTION_HEBBIAN_RATE
                    )
